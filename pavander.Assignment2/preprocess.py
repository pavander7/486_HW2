...
pattern = r"""                                 # Regular expression for tokenizing
...
def tokenizeText(text, tokenizer):
...
def preprocess_and_tokenize(text, doc_id, tokenizer="regex", verbose=0):
    """Clean, tokenize, filter, and stem raw text."""
    if verbose >= 2:
        print(f"[preprocess][L2] Starting preprocessing for '{doc_id}'.")
    
    tokens_unfiltered = tokenizeText(text, tokenizer)
    if verbose >= 3:
